...
exports.findAll = (params) => {
    let query = `SELECT ${tableName}.*, o.office_name FROM ${tableName} JOIN offices o ON o.office_id=${tableName}.office_id WHERE 1=1`
    if(params.filter){
...
exports.findAllCount = (params) => {
    let query = `SELECT count(*) as total FROM ${tableName} JOIN offices o ON o.office_id=${tableName}.office_id WHERE 1=1`
    if(params.filter){
        for(let i=0; i<params.filter.length; i++){
            const key = params.filter[i].split(':')[0]
            const value = params.filter[i].split(':')[1]
